...
function canWriteSocoop(role) {
  return role === 'socoop_admin' || role === 'president';
}

export function renderSocoop(container, me) {
  container.innerHTML = '';

  // ── 작성 폼 (매점지기·회장만 표시) ──────────────────────
  if (canWriteSocoop(me.role)) {
    let isOpen = false;
    ...
    const writePanel = el('div', { class: 'write-panel' },
      el('div', { class: 'write-panel-header' },
        el('span', { class: 'write-panel-title' }, '소쿱놀이 공지'),
        toggleBtn,
      ),
      formBody,
    );
    ...
    container.appendChild(el('div', { class: 'section-header' },
      el('h2', { class: 'section-title' }, '소쿱놀이 공지'),
    ));
  }
  ...
  const q = query(collection(db, 'socoop_notices'), orderBy('createdAt', 'desc'));
  unsubscribe = onSnapshot(q, snap => {
    listEl.innerHTML = '';
    if (snap.empty) {
      listEl.appendChild(emptyState('🏪', '아직 소쿱놀이 공지가 없습니다.'));
      return;
    }
    ...
    console.error(err);
    listEl.appendChild(emptyState('⚠️', '공지를 불러오지 못했습니다.'));
  });
}

export function unmountSocoop() {
...
function buildNoticeItem(docSnap, me) {
  ...
  const canEdit = canWriteSocoop(me.role) && (d.authorUid === me.uid || me.role === 'president');
...
function buildWriteForm(me, existing = null, onDone = null) {
  ...
  submitBtn.addEventListener('click', async () => {
    ...
    try {
      if (existing) {
        await updateDoc(doc(db, 'socoop_notices', existing.id), { title, content, updatedAt: serverTimestamp() });
        toast('공지가 수정되었습니다.', 'success');
      } else {
        await addDoc(collection(db, 'socoop_notices'), {
          title, content,
          authorUid:  me.uid,
          authorName: me.name,
...
async function handleDelete(id) {
  ...
  try {
    await deleteDoc(doc(db, 'socoop_notices', id));
